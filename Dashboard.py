...
with st.sidebar:
    ...
    years= st.multiselect("ordyr", options=df["ordyr"].unique(),default=df["ordyr"].unique())
...
filtered = df[df["Region"].isin(regions) & df["ordyr"].isin(years)]    
if submitted:
...
with tab1:
   ...
   monthly_yr=(filtered.groupby([filtered["Order Date"].dt.to_period("M").astype(str),"ordyr"])["Sales"].sum().reset_index().rename(columns={"Order Date":"Month"}))
   fig=px.line(monthly_yr,x="Month",y="Sales",color="ordyr",title="plotly grouped line:monthly sales by year")
...
with tab2:
     ...
     summary=filtered.groupby("sub-category").agg(Sales=("Sales","sum"),profit=("Profit","sum")).sort_values("Sales",ascending=False)
     st.dataframe(summary.style.format("${:,.0f}"),use_container_width=True)
     st.header("Sales vs Profit")
     fig=px.scatter(df,x="Sales",y="Profit",color="Category",size="Quantity",hover_data=["sub-category"],title="Plotly scatter Sales vs Profit")
     st.plotly_chart(fig,use_container_width=True)
with tab3:
